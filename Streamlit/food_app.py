...
with st.form("Food App"):
    st.title(":rainbow[Order Your Delicious Food !] :yellow[:material/moped_package:]", text_alignment="center")
    ...
    submit = c6.form_submit_button("Submit Your Order",type="primary", icon=":material/order_approve:")
    ...
    if submit:
        ...
        if not food_pre:
            errors.append("Please select at least one Food Preference.")
        if not food_type:
            errors.append("Please select at least one Food Type.")
        if not bev:
            errors.append("Please select at least one Beverage.")
        if not feedback.strip():
            errors.append("Feedback cannot be empty.")
        if not done:
            errors.append("You must agree before submitting.")
        ...
        if errors:
            ...
            st.header("Confirm your Order", text_alignment="center")
